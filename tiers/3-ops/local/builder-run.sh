#!/usr/bin/env bash
# Immutable scheduler entry point. The model never owns Git/GitHub delivery;
# this wrapper enters the deterministic Node controller installed alongside it.
set -euo pipefail
umask 077
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --harness) [ $# -ge 2 ] || { echo "ERROR: --harness requires a value" >&2; exit 2; }; HARNESS="$2"; shift 2 ;;
    --check) MODE="doctor"; shift ;;
...
if [ -n "$CONFIG_FILE" ]; then
  ...
  if [ -z "$HARNESS" ]; then HARNESS="$(jq -r '.harness // "claude"' "$CONFIG_FILE")"; fi
else
  # Project-installed wrapper fallback for manual Tier 2/3 use.
...
case "$MODE" in
  doctor) exec node "$CONTROLLER" doctor --harness "${HARNESS:-all}" ;;
  dry-run) exec node "$CONTROLLER" run --harness "${HARNESS:-claude}" --dry-run ;;
  run) exec node "$CONTROLLER" run --harness "${HARNESS:-claude}" ;;
esac
