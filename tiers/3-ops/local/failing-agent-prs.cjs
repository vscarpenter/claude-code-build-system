...
// via BS_TRIAGE_REPO_OWNER so same-repo branches can be told apart from forks.
...
if (require.main === module) {
  ...
  process.stdin.on("end", () => {
    ...
    process.stdout.write(String(failingAgentPRs(prs, process.env.BS_TRIAGE_REPO_OWNER).length));
  });
}
